...
build_ui <- function() {
  fluidPage(
    ...
    tags$head(
      tags$style(HTML("
        ...
        .sample-item-draggable {
          padding: 8px 12px; margin: 4px 0; background: white; border: 1px solid #bbb; border-radius: 6px;
          font-family: monospace; font-size: 12px; cursor: pointer; display: inline-block; 
          transition: background 0.2s, box-shadow 0.2s;
          user-select: none;
        }
        .sample-item-draggable:hover { background: #e3f2fd; box-shadow: 0 2px 6px rgba(0,0,0,0.1); }
        .sample-item-draggable.selected {
          background: #cce5ff;
          border-color: #3498db;
        ...
        .colourpicker-container .form-control {
          width: 60px !important;
          height: 120px !important;
        ...
        .colourpicker-container .form-control:hover {
        ...
        .input-row-with-reset {
          display: flex;
          align-items: flex-end;
          gap: 10px;
        }
        
        .input-row-with-reset .form-group {
          margin-bottom: 0;
          flex-grow: 1;
        }
      ...
      tags$script(HTML("
        ...
        $(document).on('dragstart', '.sample-item-draggable', function(e) {
          var selected = [];
          $('.sample-item-draggable.selected').each(function() {
            selected.push($(this).attr('data-sample'));
          });
        ...
        $(document).on('dragover', '.group-drop-zone', function(e) {
          e.preventDefault();
          $(this).addClass('drag-over');
        });
        $(document).on('dragleave', '.group-drop-zone', function(e) {
        ...
        $(document).on('drop', '.group-drop-zone', function(e) {
          ...
          if (data && group) {
            Shiny.setInputValue('drag_assign', JSON.stringify({samples: JSON.parse(data), group: group}), {priority: 'event'});
            $('.sample-item-draggable.selected').removeClass('selected');
          }
        });
        ...
        $(document).on('dragend', '.comparison-item', function(e) {
          $(this).removeClass('dragging');
          $('.comparison-item').removeClass('drag-over');
        });
        ...
        $(document).on('dragleave', '.comparison-item', function(e) {
          $(this).removeClass('drag-over');
        });
        ...
        $(document).on('input', '#plot_width, #plot_height', function(e) {
          ...
          if (isNaN(val) || val === 0) {
            $('#' + warningId).text('Please enter a valid positive integer');
            $(this).addClass('is-invalid');
          } else if (val > 30) {
            $('#' + warningId).text('Value too large, max recommended is 30 inches');
            $(this).addClass('is-invalid');
          } else {
            $('#' + warningId).text('');
    ...
    navbarPage(
      ...
      id = "main_navbar", 
      collapsible = TRUE, 
      windowTitle = "Proteomics Analysis",
...
# 4. 定义 server 函数，并按依赖顺序加载 server 模块
