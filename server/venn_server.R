# server/venn_server.R
message("[DEBUG] venn_server.R loading...")
...
observe({
  ...
  message("[DEBUG] venn_server: updated venn_comparisons choices, n = ", length(comp_names))
})
...
venn_data <- eventReactive(input$generate_venn, {
  ...
  message("[DEBUG] venn_data: generating data for ", reg_type, " in comparisons: ", paste(comps, collapse = ", "))
  
  protein_sets <- lapply(comps, function(comp_name) {
    res <- all_res$results[[comp_name]]
    if (is.null(res)) {
      message("[DEBUG] venn_data: comparison ", comp_name, " not found in results")
      return(character(0))
    }
    df <- res$data
    ids <- df[df$regulation == reg_type, "Master protein IDs"]
    ids <- ids[!is.na(ids) & ids != ""]
    message("[DEBUG] venn_data: ", comp_name, " - ", length(ids), " proteins")
    unique(ids)
  })
  ...
  message("[DEBUG] venn_data: generated ", length(region_list), " regions")
...
message("[DEBUG] venn_server.R loaded successfully.")