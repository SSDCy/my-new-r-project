...
# 清洗统计（仅显示 Reverse hits 和 CON_ contaminants）
output$cleaning_summary <- renderPrint({
  ...
  cat("  (Reverse hits + CON_ contaminants = ", n_reverse + n_con, ")\n", sep = "")
  
  message("[DEBUG] cleaning_summary: total_raw=", total_raw, ", total_clean=", total_clean, 
          ", removed=", n_total_removed, ", reverse=", n_reverse, ", con=", n_con)
})
...
cleaning_removed_ids <- reactive({
  ...
  if ("Master protein IDs" %in% colnames(raw) && "Master protein IDs" %in% colnames(clean)) {
    ...
    list(all_removed = character(0), reverse = character(0), con = character(0))
  }
})
...
output$cleaning_contaminant_ids <- renderPrint({
  cat("Potential contaminants are now reported as CON_ contaminants.\n")
})
