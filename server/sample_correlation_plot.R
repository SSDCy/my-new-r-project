...
message("[DEBUG] sample_correlation_plot.R loading... (Arial font in pheatmap, explanation added)")
...
# ========== 数据质量页面使用的样本相关性（log2, Z-score, 500变量） ==========
dq_sample_cor_data <- reactive({
  ...
  # 输出说明：图例显示的是 SubGroup（小分组），横纵轴是样本名
  message("[DEBUG] dq_sample_cor_data: legend shows SubGroup, axes show individual sample names (e.g., 100_6_1)")
...
output$dq_sample_cor_heatmap <- renderPlot({
  ...
  message("[DEBUG] dq_sample_cor_heatmap: rendered with Arial font")
})
...
output$download_dq_sample_cor_png <- downloadHandler(
  ...
  content = function(file) {
    dat <- dq_sample_cor_data()
    if (!is.null(dat)) {
      png(file, width = 900, height = 700, res = 150, family = "Arial")
      cor_vals <- dat$cor_mat[upper.tri(dat$cor_mat)]
      min_cor <- min(cor_vals, na.rm = TRUE)
      max_cor <- max(cor_vals, na.rm = TRUE)
      ...
      pheatmap::pheatmap(dat$cor_mat,
                         ...
                         fontfamily = "Arial")
      dev.off()
      message("[DEBUG] download_dq_sample_cor_png: file saved with Arial font")
    }
  }
)
...
output$download_dq_sample_cor_matrix <- downloadHandler(
  ...
  content = function(file) {
    ...
    if (!is.null(dat)) {
      ...
      message("[DEBUG] download_dq_sample_cor_matrix: file saved")
    } else {
...
output$sample_cor_heatmap <- renderPlot({ plot.new(); text(0.5,0.5,"This plot has been moved to Data Quality Analysis.") })
...
message("[DEBUG] sample_correlation_plot.R loaded successfully (Arial font, legend explained)")